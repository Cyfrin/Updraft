...
function setUp() public override {
    horseStore = HorseStore(HuffDeployer.config().with_args(bytes.concat(abi.encode(NFT_NAME), abi.encode(NFT_SYMBOL)))).deploy(horseStoreLocation)
}
```

Second - Our Huff constructor is super simple and really just looks like this:
...
### ERC721 and totalSupply Dispatcher/Interface

Rather than copy and pasting from the `Huffmate` repo multiple times, in the our lesson `"A Quick Function Then Huffmate"`, I included carrying over the `function dispatcher` for `Huffmate`'s `ERC721` Implementation. Review this section of the above contract carefully to assure an understanding of what was added and why.

We did miss a couple aspects of the `totalSupply` constant that we added in the previous lesson. Namely, because this constant has a getter function generated by Solidity in the back end, we need to also account for this in our interface as well as our `function dispatcher`.
