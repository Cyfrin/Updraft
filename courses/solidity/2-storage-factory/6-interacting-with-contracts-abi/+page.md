...
In this lesson, we'll upgrade the `StorageFactory` contract to keep track of all deployed `SimpleStorage` contracts and interact with them seamlessly.
...
We can now proceed to store a new number on one `SimpleStorage` contract:
...
We can then retrieve the stored value with a get function, `sfGet`:
